  ...
  return (
    <section ref={sectionRef} className="py-16 px-4">
      ...
      <div ref={titleRef} className="text-center mb-10">
        <p className="text-sm uppercase tracking-widest text-secondary-dark dark:text-slate-400 mb-2 font-medium">
          Stay connected
        </p>
        <h2 className="text-3xl md:text-4xl font-bold text-slate-800 dark:text-slate-100">
          Follow us on{" "}
          <span className="text-secondary-dark dark:text-blue-400">
            social media
          </span>
        </h2>
        <span
          ref={lineRef}
          className="mt-4 block mx-auto h-1 w-20 rounded-full bg-secondary-dark dark:bg-blue-400"
        />
      </div>
      ...
      >
        {socials.map(({ label, href, icon: Icon, color, bg, border }) => (
          <a
            ...
            className={`group flex flex-col items-center gap-3 w-36 py-7 px-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm ${bg} ${border} hover:text-white hover:shadow-xl transition-all duration-300 cursor-pointer`}
          >
            <span className="p-3 rounded-full bg-slate-100 dark:bg-slate-700 group-hover:bg-white/20 transition-colors duration-300">
              <Icon
                size={28}
                style={{ color }}
            ...
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 group-hover:text-white transition-colors duration-300">
              {label}
            </span>
          </a>
        ))}
